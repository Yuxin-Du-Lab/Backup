    ...
    * */
    ...
    public void removeUnreferenced() {
        ...
        for (int i=1; i<=getSize(); i++) {
            MyObject mo = (MyObject) elementData[i];
            if (!mo.getReferenced()) {
                // remove elementData[i] from element data
                Object first = elementData[1];
                setElementData(1, mo);
                setElementData(i, (MyObject) first);
                removeFirst();
                setSize(getSize() - 1);
                removeUnreferenced();
                return;
    ...
    public /*@pure@*/ MyObject getYoungestOne() {
        ...
        if (getSize()==0) {
            ...
            Object[] elementData = getElementData();
            Object youngest = elementData[1];
            for (int i=1; i<=getSize(); i++) {
                if (((MyObject)elementData[i]).compareTo((MyObject) youngest)<0) {
                    youngest = elementData[i];
                }
            }
            return (MyObject) youngest;
        }
    }
}
