...
public class PathContainer {        // 包含题目[2]-[9]
    ...
    private HashMap<Path, Integer> pathList;        //对应规格中的pList, pidList两个数组
    private HashMap<Integer, Path> pathIdList;      //仍然对应规格中的pList, pidList两个数组

    private /*@spec_public@*/ int idCounter;
    ...
    /*@ normal_behavior
      ...
      @           (\exist int j; 0 <= j && j < pList.length; \old(pList[i]).equals(pList[j]) && \old(pidList[i]) == pidList[j]));
    ...
    /*@ public normal_behavior
      ...
      @          (\exist int j; 0 <= j && j < pList.length; \old(pList[i]).equals(pList[j]) && pidList[i] == pidList[j]));
      ...
      @ ensures (\forall int i; \old(getPathById(pathId).containsNode(i));
      @          this.getNodeCount(i) == \old(this.getNodeCount(i)) - \old(getPathById(pathId).getNodeCount(i)));
      @ also
      @ public exceptional_behavior
      @ assignable \nothing;
