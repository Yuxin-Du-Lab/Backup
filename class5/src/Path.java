...
public class Path implements Iterable<Integer> {        // 包含题目[10]-[14]
    ...
    /*@ public normal_behavior
      ...
      @*/
    /*
     @ signals (IndexOutOfBoundsException e) index < 0 || index >= size(); //TODO
     */
    ...
    //@ ensures \result == (\exists int i; 0<=i && i<nodes.length; nodes[i].id == nodeId);    //TODO
    ...
    /*@ ensures (\exists int[] arr; (\forall int i, j; 0 <= i && i < j && j < arr.length; arr[i] != arr[j]）;
      ...
      @            && (\forall int i; 0 <= i && i < nodes.length; (\exists int j; 0 <= j && j < arr.length; nodes[i] == arr[j]))
      @            && \result == arr.length);
      @*/
    ...
    /*@ ensures \result == (nodes.length >= 2) &&
      @                     ((\sum int i; 0 <= i && i < nodes.length - 1 && (\exist int j; i < j && j < nodes.length; nodes[i] == nodes[j]); 1) <= 1);
      */
    public /*@pure@*/ boolean isValid() {
        //return (size() >= 2 && [13]);   //TODO  请使用类中的属性(及其方法)和运算符实现
        return (size() >= 2 && (size() - getDistinctNodeCount()) <= 1);   //TODO
    }
    ...
    public /*@pure@*/ Path extractLoopPath() {
        ...
        //[14]; //TODO
        loopStartIndex = nodeToIndex.get(nodes.get(index)); //TODO
        loopLastIndex = index;
