...
public class MyUmlRegion {
    ...
    public void checkForUml008() throws UmlRule008Exception {
        for (UmlPseudostate initialState : id2pseudostate.values()) {
            String initialStateId = initialState.getId();
            boolean findOut = false;
            for (MyUmlTransition myTransition : id2transition.values()) {
                UmlTransition transition = myTransition.getOrigin();
                if (transition.getSource().equals(initialStateId)) {
                    if (findOut) {
                        throw new UmlRule008Exception();
                    } else {
                        findOut = true;
                        if (transition.getGuard() != null || myTransition.getTriggersSize() != 0) {
                            throw new UmlRule008Exception();
                        }
                    }
                }
