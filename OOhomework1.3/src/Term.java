...
public class Term {
    ...
    public Term(boolean negativeIn, Constant constantIn) {
        if (negativeIn) {
            constant = constantIn;
            constant.update(new Constant("-1", false));
        } else {
            constant = constantIn;
        }
    }
    ...
    public void addFactors(Factor factor) {
        if (factor.isNegative()) {
            constant.update(new Constant("-1", false));
            //System.out.println("POWER_FACTOR negative CHECKED!");
        }
    ...
    public Expression diff() {
        ...
        for (int i = 0; i < expressions.size(); i++) {
            Term diffTerm = new Term(false, new Constant("1", false));
